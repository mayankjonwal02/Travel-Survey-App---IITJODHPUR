...
import com.iitj.travelsurveyappiitj.databinding.FragmentEndpointBinding
...
class endpointFragment : Fragment(),  OnMapReadyCallback {
    ...
    override fun onCreateView(
        ...
        binding= FragmentEndpointBinding.inflate(inflater,container,false)
        binding.endmap.onCreate(savedInstanceState)
        binding.endmap.onResume()
        binding.endmap.getMapAsync(this)
        return binding.root
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.submit.setOnClickListener(){
            if(binding.address.text.toString().isNotEmpty())
            {
                ...
                refer.child("EndLocation").child("address").setValue(binding.address.text.toString())
                navController.navigate(R.id.action_endpointFragment_to_letsstart)
            }
            else
            {
    ...
    private fun getaddress() {
        ...
        task.addOnSuccessListener {
            if (it != null) {
                ...
                binding.address.setText(addr[0].getAddressLine(0))
                refer.child("EndLocation").child("address").setValue(binding.address.text.toString())
                refer.child("EndLocation").child("latitude").setValue(it.latitude)
                refer.child("EndLocation").child("longitude").setValue(it.longitude)
                map.addMarker(MarkerOptions().position(LatLng(it.latitude,it.longitude)).title("your location"))
